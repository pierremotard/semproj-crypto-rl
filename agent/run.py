...
class Run(object):
    ...
    def train_agent(self, episode_number=0, eps_start=1.0, eps_end=0.01, eps_decay=0.996):
        """
            Params
            ======
        ...
        for episode_step in range(1, self.number_of_training_steps):

            state = torch.Tensor(state).unsqueeze(dim=0).view(
                1, -1).to(device)  # Add a first dim, 1 to batch_size
            ...
            if episode_step % update_timestep == 0:
                print(f"Timestep {local_step_number}, optimize")
    ...
    def test_agent(self, day_nb=0):
        ...
        for step in range(17030):
            state = torch.Tensor(state).unsqueeze(dim=0).view(1, -1).to(device)
            amount, action_type = self.agent.act(state)
